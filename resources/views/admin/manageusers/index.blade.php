...
@section('dashboard')
    ...
    <div class="d-flex flex-column-fluid">
        ...
        <div class="container" >
            ...
            <div class="card">
                <form class="form" method="post" action="{{url('admin/give-permissions')}}">
                    @csrf
                    <div class="card-body">
                        <h4>Give Permission To User</h4>
                        <br>
                        <br>
                        <div class="form-group row">
                            <br>
                            <div class="col-lg-9 col-md-9 col-sm-12">
                                <select class="form-control @error('user') is-invalid @enderror" onchange="show_permission()" name="user" id="user" required>
                                    <option disabled selected readonly>Select User</option>
                                    @foreach($users as $user)
                                        @if(!$user->hasRole('Admin'))
                                        <option value="{{$user->id}}">Name:{{$user->name}} -- Email:{{$user->email}} -- Role:{{$user->roles->pluck('name')}}</option>
                                        @endif
                                    @endforeach
                                </select>
                                @error('user')
                    ...
                    <div id="show_perm" style="overflow:auto;">
...
@section('js')
 <script>
     ...
     function show_permission() {
         ...
         $.ajax({
            type:'POST',
            url:"{{route('admin.show.permissions')}}",
             cache:false,
             data:{user_id:user, _token: v_token},
